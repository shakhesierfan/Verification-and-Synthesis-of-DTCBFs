...
def verification_fcn(A, B, C, D, E, F, Ts, gamma):
...
# --- Computing alpha_F_1 and alpha_F_2 (for convexifying the objective function)    
    ...
    def draw_rect_colored(th_L, th_U, om_L, om_U, u_star, *, edge='k', lw=0.8, alpha=0.8):

        u_clipped = float(np.clip(u_star, vmin, vmax))
        color = cmap(norm(u_clipped))
        r = Rectangle((th_L, om_L), th_U - th_L, om_U - om_L,
...
verification_fcn(-2.55, -2.55,  -1.6711,   0.1,   0.1,   1, 0.01, 1)
